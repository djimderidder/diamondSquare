...
%         surface = S(e:N+e-1,e:N+e-1);subplot(2,4,2*i-1); heatmap(surface);
        for j=1:2^(i-1)
            for k=1:2^(i-1)
                x1 = ((j-1)*x)+e;
                y1 = ((k-1)*x)+e;
...
%         surface = S(e:N+e-1,e:N+e-1);subplot(2,4,2*i); heatmap(surface);
        range = range*2^(-h);
    end
    ...
    title(['n = ',num2str(n),'; h = ',num2str(h),'; initial range RV = ',num2str(irange)]);
    ax = gca;
